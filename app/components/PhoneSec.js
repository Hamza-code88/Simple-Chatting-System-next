...
const Phone1 = ({
  ...
  const handleSend = async () => {
    if (message.trim() !== "") {
      ...
      setTimeout(() => {
        handleSendMessage(); // Send the message after delay
  ...
  return (
    <div className="relative w-[320px] h-[600px] bg-white rounded-3xl shadow-xl border border-gray-300 overflow-hidden">
      <div className="absolute top-4 left-1/2 transform -translate-x-1/2 w-12 h-12 bg-gray-600 rounded-full border-4 border-white"></div>{" "}
      {/* Camera Style */}
      <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-16 h-2 bg-black rounded-full z-10"></div>
      <div className="absolute inset-0 mt-4 mb-2 mx-4 bg-white rounded-xl flex flex-col p-4 space-y-4">
